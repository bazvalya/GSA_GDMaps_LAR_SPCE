...
class Trader(mesa.Agent):
    ...
    def __init__(
        ...
        self.prices = []
        self.trade_partners = []
        self.dead = False  # Flag to track if agent's sugar or spice reached zero
    ...
    def trade(self, other):
        ...
        # print("Agent ID:", self.unique_id, "Prices:", np.exp(np.log(self.prices).mean()))
    ...
    def eat(self):
        if not self.dead: 
            ...
            # Print agent ID and sugar level
        # print("Agent ID:", self.unique_id, "Sugar:", self.sugar, "Spice:", self.spice)
    ...
    def phantom_death(self):
        """
        Function to remove Traders who have consumed all their sugar or spice
        """
        if self.is_starved() and not self.dead:
            self.dead = True
            self.model.grid.remove_agent(self)
            # self.model.schedule.remove(self)
