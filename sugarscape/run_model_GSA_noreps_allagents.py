...
fixed_parameters = {'initial_population': 100,
                    'width': 50,
                    'height': 50,
                    'endowment_min': 25,
                    'endowment_max': 50,
...
def run_model(args):
    model, args, problem_sampler, param_combos, fixed_parameters, i = args
    # create dictionary with variable parameters
    var_param = {}
    for key, val in zip(problem_sampler['names'], param_combos.T):
        var_param[key] = val

    m = model(**var_param, **fixed_parameters, seed=None)
...
def run_model_parallel(args):
    ...
    with mp.Pool(n_cores) as pool:
        for p, model in tqdm.tqdm(pool.imap_unordered(
            run_model,
            ...
            results_agents[p] = model.datacollector.get_agent_vars_dataframe().to_dict()
